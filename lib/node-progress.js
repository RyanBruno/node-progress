...
function ProgressBar(fmt, options) {
  ...
  this.stream = Array.isArray(this.stream) ? this.stream : [this.stream];
...
ProgressBar.prototype.render = function (tokens) {
  ...
  var minColumns = this.stream[0].columns;
  for (let i = 1; i < this.stream.length; i++) {
    if (minColumns > this.stream[i]) {
      minColumns = this.stream[i];
    }
  }
  var availableSpace = Math.max(0, minColumns - str.replace(':bar', '').length);
